...
// shared wall hugging band: arc sampled stations cut around the open sectors
abstract class SlideBandProvider : SlideAttachmentModelProvider() {

    companion object {
        internal const val FRAME = 0.11
    ...
    protected open fun signatureExtra(ctx: SlideAttachmentModelContext): String = ""

    protected open fun extraParts(
    ...
    final override fun boundingBox(ctx: SlideAttachmentModelContext): AABB {
        ...
        for (part in parts(ctx)) {
            val quads = when (part) {
                is PolygonPart -> part.quads
                is TiledPart -> part.quads
                else -> continue
            }
            for (q in quads) expand(q, b)
        }
    ...
    private fun hubBox(ctx: SlideAttachmentModelContext): AABB {
        ...
        return AABB(
            ox - r, oy - r, -backDistance(ctx.data),
            ox + r, oy + r, frontDistance(ctx.data)
        )
    }

    final override fun parts(ctx: SlideAttachmentModelContext): List<Part> {
        val left = backDistance(ctx.data)
        val right = frontDistance(ctx.data)
        ...
        val spans = wallSpans(sectors, WaterslideTubeMesh.crossSections())
        val quads = ArrayList<Quad>()
        for (span in spans) emitSpan(stations, span, quads)
        val out = ArrayList<Part>(2)
        if (quads.isNotEmpty()) out.add(PolygonPart(quads))
        out.addAll(extraParts(ctx, stations, spans))
        cache[key] = out
        return out
    }
    ...
    protected fun distance(a: Station, b: Station): Double {
